...
## Get the Linux version while the PR is still a draft

The Linux files are currently on the `feat/linux-support` branch, not `main`.
Clone that exact branch:

```bash
git clone --branch feat/linux-support --single-branch \
  https://github.com/henryrts/LegionGen10dGPU-script.git
cd LegionGen10dGPU-script/Linux
```

After the Linux PR is merged, a normal clone of `main` will be sufficient.
...
- If the path exists, clone the draft branch and continue with `install.sh`.
